    ...
    <div className="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none" style={{ maxWidth: '340px' }}>
      {toasts.slice(-5).map((toast) => (
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, #1a0533 0%, #150228 100%)',
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div style={{ color: '#ddd6fe' }} className="text-xs font-semibold truncate">
              {toast.title}
            </div>
            <p style={{ color: '#7c5fa0' }} className="text-xs mt-0.5 truncate">
              {toast.message}
            </p>
          </div>
          <button
